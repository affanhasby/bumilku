...
.screen{flex:1;overflow-y:auto;overflow-x:hidden;padding-bottom:calc(88px + env(safe-area-inset-bottom));-webkit-overflow-scrolling:touch;scrollbar-width:none;overscroll-behavior:none;}
.screen::-webkit-scrollbar{display:none;}
/* Nav */
.bnav{position:fixed;bottom:0;left:50%;transform:translateX(-50%);width:100%;max-width:430px;background:var(--wh);border-top:1px solid var(--bd);display:flex;z-index:200;padding:10px 0 0;padding-bottom:max(10px,env(safe-area-inset-bottom));box-shadow:0 -6px 24px var(--sh2);transition:transform .25s ease,opacity .25s ease;}
body.sheet-open .bnav{transform:translateX(-50%) translateY(100%);opacity:0;pointer-events:none;}
.ni{flex:1;display:flex;flex-direction:column;align-items:center;gap:3px;cursor:pointer;border:none;background:none;color:var(--mu);padding:6px 0 4px;transition:color .2s;font-family:'Plus Jakarta Sans',sans-serif;}
.ni.on{color:var(--ro);}
...
/* Header */
.hdr{background:linear-gradient(150deg,var(--ro2) 0%,var(--ro) 45%,var(--pe) 100%);padding:calc(env(safe-area-inset-top) + 20px) 20px 28px;position:relative;overflow:hidden;}
